 ...
 * We learn the ratio online per geohash cell and pre-correct the next request:
 * requested = target / factor.
 */
...
export class ScaleFactorTable {
  ...
  get(lat: number, lon: number): number {
    return this.factors.get(this.key(lat, lon)) ?? 1.0;
  }

  /** Distance to request from the engine so the realized length lands near targetM. */
  correctedRequest(lat: number, lon: number, targetM: number): number {
    return targetM / this.get(lat, lon);
  }

  /** Fold one observation (requestedM → realizedM) into the area's EWMA. */
  observe(lat: number, lon: number, requestedM: number, realizedM: number): void {
  ...
  private key(lat: number, lon: number): string {
    return geohashEncode(lat, lon, this.precision);
  }
}
